...
| `doc.generatedAt` | `DocGeneratedAt` | — | `date`, `string`, `text` | Нет | «Дата публикации» — автозаполняется датой генерации документа |
| `doc.generatedBy` | `DocGeneratedBy` | — | `string`, `text` | Нет | «Публикатор» — автозаполняется именем пользователя, запустившего генерацию |
...
| Группа | Область |
|---|---|
| `doc.*` | Реквизиты и метаданные документа (номер, дата, генерация, печатная форма) |
