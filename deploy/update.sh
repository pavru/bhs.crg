...
do_check() {
    ...
    local latest; latest="$(latest_release)"
    ...
    say "Установлена версия $CURRENT, новее ничего нет."
    exit 3
}
...
do_gc() {
    ...
    local repos
    repos="$(grep -oE 'image:[[:space:]]*ghcr\.io/[^:[:space:]]+' docker-compose.yml | sed -E 's/^image:[[:space:]]*//' | sort -u)"
    [ -n "$repos" ] || stop \
    ...
    # версии — это готовый откат, и снести образ из-под него значит отобрать откат молча.
