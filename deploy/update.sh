...
#
# Хорошая новость, проверенная на живых контейнерах: молча ничего не случится. Со старым
# монтированием образ 18 НЕ СТАРТУЕТ и печатает причину — и с пустым томом, и с кластером 16.
# То есть цена слепого обновления — не потерянные данные, а система, которая не поднялась.
# Плохая новость: узнать об этом посреди обновления, на боевом сервере, дороже всего.
#
...
система будет лежать, пока вы не вернёте прежний выпуск.

Процедура — §8.6 DEPLOYMENT.md «Переход на PostgreSQL 18». Коротко: снять дамп работающей базой,
поднять новую на 18 (том другой, прежний остаётся страховкой), восстановить дамп, и только потом
поднимать остальное. Занимает минуты, но требует, чтобы вы были у консоли.
EOF
)"
}
