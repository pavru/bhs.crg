...
prepare_backups() {
    ...
    if [ "$(id -u)" = 0 ]; then
        ...
        # ОСТАНОВКА, а не предупреждение (issue #896): система с чужим каталогом копий не стартует,
        # и это известно ЗДЕСЬ — до `up -d`. Предупредить и пойти дальше значило бы отправить
        # человека ждать пять минут готовности, которая не наступит, и разбирать журнал вместо
        # выполнения одной команды.
        stop "$(cat <<EOF
Каталог $dir создан, но сменить владельца без прав root не вышло. Система с ним НЕ ЗАПУСТИТСЯ:
сервер работает под пользователем app (uid 1654) и откажется писать в чужой каталог.

Выполните и повторите установку — всё, что уже скачано, останется на месте:
    ...
    while IFS='|' read -r name state; do
        ...
        case "$state" in
            restarting|exited|dead)
                # garage-init выходит с нулём по устройству — это одноразовый сервис, и его
                # «exited» не поломка. Отличаем по коду выхода, а не по имени.
                if [ "$name" = garage-init ]; then
                    [ "$(docker inspect -f '{{.State.ExitCode}}' "$(compose ps -aq "$name" 2>/dev/null | head -1)" 2>/dev/null || echo 1)" = 0 ] && continue
                fi
...
' "$broken" >&2
    ...
    printf '
Система не ответила за пять минут, но контейнеры работают. Журнал api:
' >&2
