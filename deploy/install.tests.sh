...
echo
echo '── Запрос к себе идёт МИМО прокси (issue #896) ──'
# На проде с `all_proxy=socks5://…` curl уводил в прокси даже обращение к петле: установка вставала
# на «Ждём готовности системы» при работающем стеке, а следом молча не создавался администратор.
# Проверяем функционально: запрос к заведомо закрытому локальному порту обязан упереться в отказ
# СОЕДИНЕНИЯ, а не в прокси.
# `-sS`, а не `-s`: тихий режим глушит и текст ошибки, и тогда проверка сравнивала бы пустоту с
# пустотой — она проходила бы и со сломанным curl_local (проверено).
out="$(all_proxy=socks5://127.0.0.1:1 curl_local -sS --max-time 3 http://127.0.0.1:9/ 2>&1 || true)"
check 'ответ не про прокси'  нет "$(case "$out" in *'over proxy'*|*'proxy'*) echo да ;; *) echo нет ;; esac)"

# И статически: чтобы следующий локальный запрос не появился в обход curl_local. Ищем строки, где
# обычный curl обращается к петле, — их быть не должно ни в одном из двух скриптов.
# ⚠️ Имя переменной — НЕ `bad`: так называется счётчик обвязки, и присваивание ему обнуляло итог.
# Проверено: набор печатал «ФЕЙЛ короткий отвергнут» и тут же «фейлов 0», возвращая ноль, — то
# есть все проверки выше становились необязательными, а CI зеленел на настоящих поломках.
#
# Ищем не по строке с адресом (в install.sh адрес лежит в переменной, и такой поиск слеп), а от
# обратного: КАЖДЫЙ curl в этих скриптах обязан быть либо curl_local, либо запросом наружу.
stray=0
for f in "$SRC" "$(dirname "$SRC")/update.sh"; do
    found="$(awk '
        # Простой /curl /: у `curl_local` после «curl» идёт подчёркивание, значит эта строка сюда
        # не попадает. Группировка `(^|[^_[:alnum:]])` в здешнем awk работает не так, как ожидалось,
        # и пропускала вызовы вида `$(curl …` — ровно те, что и надо ловить (проверено).
        /curl / && $0 !~ /curl_local/ && $0 !~ /^[[:space:]]*#/ {
            ctx = $0
            # Вызов может быть перенесён: адрес тогда на следующей строке. Смотрим пару.
            # Проверяем последний символ напрямую: регулярка с обратным слэшем здесь
            # разваливалась («unterminated regexp»), awk падал, и сторож МОЛЧАЛ всегда.
            if (substr($0, length($0), 1) == "\\") { if ((getline nxt) > 0) ctx = ctx " " nxt }
            if (ctx !~ /RELEASE_URL|LATEST_URL|ghcr[.]io|github[.]com/) print "       " substr(ctx, 1, 90)
        }
    ' "$f")"
    if [ -n "$found" ]; then
        stray=$((stray + 1))
...
check 'все curl — либо curl_local, либо наружу' 0 "$stray"
