...
out="$(all_proxy=socks5://127.0.0.1:1 curl_local -s --max-time 3 http://127.0.0.1:9/ 2>&1 || true)"
...
for f in "$SRC" "$(dirname "$SRC")/update.sh"; do
    grep -nE '(^|[^_])curl [^|]*(127\.0\.0\.1|localhost)' "$f" | grep -v curl_local > /dev/null 2>&1 && bad=$((bad + 1))
done
check 'прямых curl к петле не осталось' 0 "$bad"
