...
<xsl:stylesheet
	...
	<xsl:template match="*[bf:InstanceOf(.,'urn:BimHouse:CommonDataType','Тип.Базовый.Документ.Приложение.ВедомостьМатериаловГруппировка')]" mode='subtype-presenting'>
		<xsl:element name='ct:Обработки' namespace='urn:BimHouse:CommonDataType'>
			<xsl:element name='ct:ДокументыКачества' namespace='urn:CommonDataType'>
				<xsl:for-each-group select='ct:Материалы/ct:Группа/ct:Материал/ct:ДокументПодтверждающийКачество' group-by="lower-case(concat(normalize-space(ct:ТипДокумента),':',ct:НомерДокумента,':',ct:ДатаДокумента))">
