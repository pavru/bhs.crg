...
export interface DocumentType {
  ...
  /** Владелец типа (ТЗ CORE-18): код модуля или `core`. Типы выключенного модуля не предлагаются. */
  module: string;
  /** Где лежат объекты типа: общая таблица или таблица модуля (ТЗ CORE-16). */
  storage: 'SharedObject' | 'ModuleTable';
  /**
   * Насколько администратор правит схему (ТЗ CORE-19). Объявляет модуль для своих типов; тип,
   * заведённый человеком, открыт. ⚠️ Замки в редакторе (системные поля, запрет действий) — забота
   * следующей задачи: сегодня все типы открыты, и рисовать замок было бы не на чем.
   */
  editLevel: 'Open' | 'Extendable' | 'Closed';
  /**
   * Видно ли объекты типа за пределами модуля-владельца. ⚠️ Признак заведён заранее и СЕГОДНЯ НЕ
   * ДЕЙСТВУЕТ ни на один путь чтения — включается работой STG-11. В интерфейсе его поэтому нет:
   * переключатель, который ничего не меняет, — отказ, переодетый в результат.
   */
  visibility: 'Closed' | 'Shared';
  /** Каналы чтения, открытые закрытому типу. Там же, где видимость: пока не действует. */
  readChannels: string[];
