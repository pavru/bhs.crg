...
export interface SchemaField {
  ...
  expression?: string;
  /**
   * Происхождение поля (ТЗ CORE-19.1, issue #956): `module` — поле завёл МОДУЛЬ, у него замок;
   * отсутствие свойства — поле заказчика.
   *
   * ⚠️ Свойство обязано пережить круг «разобрали → показали → сохранили». Редактор пересобирает
   * поля поимённо, и пропусти он `origin` — единственный редактор схемы стирал бы разметку модуля
   * при каждом сохранении: у закрытого типа даже разрешённая правка подписи упиралась бы в отказ
   * «нельзя убрать происхождение», а у открытого замок исчезал бы молча. Найдено ревью PR #1004.
   */
  origin?: 'module' | 'customer';
  /**
   * Замок поля (ТЗ CORE-20.2, issue #957): значение кладёт код модуля, а не форма. Охрана записи
   * требует оставить такое поле бит-в-бит; редактор типа не даёт ни поставить метку, ни снять.
   *
   * ⚠️ То же требование круга, что у `origin`, и пропущено оно было ровно так же (issue #1008):
   * без этого свойства редактор отправляет схему БЕЗ замка, сервер отвечает «нельзя снять замок» —
   * а проверка замка стоит до раннего выхода по уровню, то есть тип с запертым полем нельзя
   * сохранить вообще, даже правя чужое поле. Сторож круга — `schemaRoundTrip.test.ts`.
   */
  locked?: boolean;
  /**
   * Подпись поля, как её объявил МОДУЛЬ (issue #958). Сравнением с `title` проекция отличает правку
   * администратора (она остаётся) от подписи, которую сама же положила в прошлый раз (её заменяет
   * новая версия модуля). В интерфейсе не показывается — это служебная запись.
   *
   * ⚠️ Третье свойство с требованием круга, и терялись предыдущие два ровно так же (#1004, #1008).
   * Пропусти его пересборка — подпись КАЖДОГО поля модуля навсегда числилась бы правленной
   * человеком, и правка подписи в новой версии модуля не доехала бы до установки никогда, молча.
   */
  moduleTitle?: string;
}
