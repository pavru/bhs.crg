 ...
 */
/*
 * НЕ входят сюда сознательно (issue #759, где этот фильтр впервые заработал — до него сервер не
 * отдавал `code`, и набор не значил ничего):
 *
 * - `quality-out-of-scope` (#733). Кандидат по букве правила выше, но плитка несёт НОТУ
 *   «целевой объект не найден или удалён», а он на месте — просто вне области видимости комплекта.
 *   Пометить его этой плиткой значит объяснить человеку неправду и послать искать пропажу. Нужен
 *   свой текст ноты, а это решение #733.
 * - `source-field-empty` (#762). Не битая ссылка вовсе: документ-источник цел, поле в нём ещё не
 *   заполнили. Это незаконченная работа, и она уже сказана предупреждением в панели.
 */
