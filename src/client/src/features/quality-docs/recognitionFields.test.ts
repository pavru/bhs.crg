...
describe('codesFromLabels', () => {
  ...
  it('нестроковое значение проходит насквозь', () => {
    // issue #1005: число приходит уже числом, и искать по нему код перечисления не только
    // бессмысленно — `normalize` на числе упал бы.
    expect(codesFromLabels({ 'Листов': 3 }, plan.enumCodes)).toEqual({ 'Листов': 3 });
  });
