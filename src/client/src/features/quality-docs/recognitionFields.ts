...
export function buildRecognitionFields(
  ...
  for (const f of fields) {
    ...
    /**
     * Запертое поле (ТЗ CORE-20.2) модели не показываем вовсе — ни спросить, ни записать.
     *
     * Ответ модели уходит прямо в реквизиты, а охрана записи требует оставить запертое поле
     * бит-в-бит: распознай мы его, сохранение получило бы 400 «Поле заперто» — и вернуть прежнее
     * значение было бы НЕЧЕМ, контрол у такого поля read-only. Человек терял бы вместе с ним все
     * остальные распознанные значения, закрывая форму.
     *
     * Здесь, а не в формах: план строят три места — форма документа качества, форма записи общих
     * данных и безголовый импорт (`recognizeImported`), где формы нет вовсе и отказ увидеть некому.
     */
