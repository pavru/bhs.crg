    ...
    <div className="border border-stroke rounded-lg overflow-hidden">
      ...
      {compositeType && (
        <RefPickerModal
          ...
          // «Организация».
          //
          // Инвариант union'а держится: раздел берёт только поля type='complex' РОВНО того же типа,
          // то есть до doc-ref-варианта дело не доходит, а поле типа самого union'а кладётся без
          // ключа варианта — то же, что placeInUnion возвращает для kind:'self'.
          //
          // А вот «ссылка на ссылку» из #750 ВОЗМОЖНА, только не там, где её искала задача. Поле
