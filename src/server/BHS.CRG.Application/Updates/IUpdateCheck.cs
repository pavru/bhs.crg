...
{
    /// <summary>
    /// Состоялась ли ПОСЛЕДНЯЯ попытка (заполняется только ответом на явную проверку).
    /// null — не спрашивали, просто читали известное.
    ///
    /// Существует потому, что иначе неудача выглядит как успех: служба глотает сбой сети,
    /// возвращает прежнее состояние, а кнопка «Проверить сейчас» отвечает 200 и не меняет на экране
    /// ничего. Ровно тот отказ, переодетый в результат, ради которого кнопка и заведена: она должна
    /// делать выключатель «включено» опровержимым.
    /// </summary>
    public bool? JustChecked { get; init; }

    /// <summary>Почему последняя попытка не удалась — человеку, а не в журнал.</summary>
