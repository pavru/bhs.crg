...
/// <summary>
/// Пакетный разрыв связей по идентификаторам (issue #556). Отдельная команда, а не цикл на клиенте:
/// на живых данных у одного сертификата 69 связок, и снимать их 69 запросами — не вариант.
///
/// Асимметрия с созданием осознанная: создание пакетное ПО КЛЮЧАМ (upsert из контекста, где Id ещё
/// нет — материалы набора данных), удаление — ПО ID (из контекста, где строка уже перед глазами).
/// </summary>
public record RemoveMaterialLinksCommand(IReadOnlyList<Guid> Ids) : IRequest<int>;
