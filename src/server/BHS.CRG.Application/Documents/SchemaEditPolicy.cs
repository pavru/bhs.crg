...
/// <summary>
/// Подпись поля, КАК ЕЁ ОБЪЯВИЛ модуль (issue #958). Хранится рядом с той, что видит человек.
///
/// <para>Зачем вторая подпись. Администратору оставлена правка подписи даже в закрытом типе, и она
/// обязана пережить перезапуск. Но «подпись из базы всегда побеждает» означает другое: смена
/// подписи в НОВОЙ версии модуля не доедет до существующих установок никогда — и отличить правку
/// администратора от подписи, которую сама же проекция положила в прошлый раз, без второй записи
/// невозможно. Сравнение с ней и даёт ответ: <c>title</c> разошлась с <c>moduleTitle</c> — это
/// правка человека, она остаётся; совпала — человек её не трогал, и приезжает новая.</para>
///
/// <para>⚠️ Метка обязана пережить круг «разобрали → показали → сохранили» на клиенте. Потеряй его
/// пересборка эту метку — подпись любого поля модуля стала бы «правленной администратором» навсегда
/// (issue #1008: ровно так уже потерялся <c>locked</c>). Стережёт <c>schemaRoundTrip.test.ts</c>.</para>
/// </summary>
public static class SchemaFieldModuleTitle
{
    /// <summary>Имя свойства в JSON поля.</summary>
    public const string Property = "moduleTitle";
}
