...
/// её, а звать в цикле для него естественно. Спрашивается с указанием ВИДА: у комплекта и
/// набора целей-двойников хватает, и без сужения идущая сверка отвергала бы сборку;</item>
...
/// открытым (issue #801). Но ПОСЛЕ проверки существования цели: неверный идентификатор,
/// получивший ответ «распознавать некому», отправляет разбираться с настройками движка вместо
/// опечатки в адресе;</item>
...
/// </list>
///
/// Окно между проверкой «уже идёт» и постановкой задачи закрыто уникальным индексом в базе
/// (issue #900): проигравший гонку получает тот же отказ, только вынесенный базой, у которой
/// промежутка между проверкой и записью нет. Проверка при этом остаётся — она отвечает раньше и
/// текстом, называющим операцию, а индекс страхует случай, когда двое пришли одновременно.
///
