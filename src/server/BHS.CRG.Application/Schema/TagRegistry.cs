...
public static class TagRegistry
{
    ...
    [
        ...
        // Ссылка на классификатор. Носитель у неё есть с этой же задачи: миграция добавляет
        // необязательное поле «Вид работы» в тип «Работа» (ТЗ TYPE-7.1).
        new(FunctionalTag.RefWorkType, "Ссылка на вид работы",
            "Поле ссылается на запись классификатора видов работ. По ТЗ CORE-13 смета, график и отчёт обязаны ссылаться на вид работы, а не склеиваться по совпадению названия — тэг и есть то, по чему код находит эту ссылку, не зная названия поля.",
