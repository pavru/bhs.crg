...
public static class GostTableFields
{
    ...
    // Кабельный журнал — суперсет-колонки (issue #389), покрывают формы 4 ГОСТов (21.613 силовое,
    // 21.608 внутр. освещение, 21.607 наружное освещение, Р 70444) как подмножества. Ключевое —
    // ДВОЙНАЯ форма «По проекту / Проложен(о)»: у каждой секции своя Марка/Сечение/Длина (пары *Проект/*Факт).
    // Простые одиночные формы заполняют только *Проект. Все string (форматирует шаблон, #60).
    public static readonly IReadOnlyList<RecognitionField> CableJournalColumns =
    [
        new("НомерКабеля", "Номер (обозначение) кабеля/провода/линии", "string"),
        new("Начало", "Начало линии (откуда): пункт/щит/опора", "string"),
        new("Конец", "Конец линии (куда): пункт/щит/опора", "string"),
        new("Участок", "Участок / № группы (освещение) / пролёт опор (наружное); для силового обычно пусто", "string"),
        new("МаркаПроект", "Марка кабеля/провода ПО ПРОЕКТУ", "string"),
        new("СечениеПроект", "Число и сечение жил (напряжение) по проекту", "string"),
        new("ДлинаПроект", "Длина по проекту, м", "string"),
        new("МаркаФакт", "Марка проложенного ФАКТИЧЕСКИ", "string"),
        new("СечениеФакт", "Число и сечение жил фактически", "string"),
        new("ДлинаФакт", "Длина фактически, м", "string"),
        new("СпособПрокладки", "Способ прокладки / характеристика трассы", "string"),
        new("Примечание", "Примечание", "string"),
    ];
