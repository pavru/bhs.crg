...
public static class RecognitionShared
{
    ...
    /// <summary>Промпт распознавания КАБЕЛЬНОГО (кабеле-проводникового) журнала (issue #389). В отличие
    /// от общего <see cref="BuildTablePrompt"/> — описывает ДВОЙНУЮ форму «По проекту / Проложен(о)»,
    ...
    public static string BuildCableJournalPrompt(IReadOnlyList<RecognitionField> fields, RecognitionTableShape? shape)
    {
        ...
        sb.AppendLine("документации по ГОСТ (21.613 силовое эл-оборудование, 21.608 внутреннее освещение,");
        sb.AppendLine("21.607 наружное освещение, Р 70444). Форма БЫВАЕТ ДВОЙНОЙ:");
        sb.AppendLine("- «Трасса»: Начало (откуда) и Конец (куда) линии.");
        sb.AppendLine("- «Кабель/провод» может делиться на ДВЕ секции: «По проекту» и «Проложен(о)» (фактически");
        sb.AppendLine("  проложенный). У КАЖДОЙ секции свои Марка, Число и сечение жил (напряжение), Длина.");
        sb.AppendLine("МАППИНГ КОЛОНОК (строго):");
        sb.AppendLine("- Значения секции «По проекту» → поля *Проект (МаркаПроект/СечениеПроект/ДлинаПроект).");
        sb.AppendLine("- Значения секции «Проложен(о)»/«Фактически» → поля *Факт (МаркаФакт/СечениеФакт/ДлинаФакт).");
        ...
        out IReadOnlyDictionary<string, string?> values, out string? problem)
    {
        ...
        catch (JsonException ex)
        {
            ...
            // Текст System.Text.Json наружу не идёт (issue #1050): отсюда он попадает в
            // RecognitionSilentException, а его сообщение доходит до человека дословно. Полезное из
            // него — только место обрыва, и называется оно своими словами.
            //
            // Место называется ЦЕЛИКОМ или не называется вовсе: BytePositionInLine — смещение
            // внутри текущей СТРОКИ, и одно оно отправляло бы читателя в начало ответа всякий раз,
            // когда модель отдала JSON с переносами. Оба числа нумеруются с нуля — прибавляем
            // единицу, чтобы совпадать с нумерацией в редакторе.
            problem = ex.LineNumber is { } line && ex.BytePositionInLine is { } at
                ? $"ответ не разобран как JSON (оборвался на строке {line + 1}, позиция {at + 1}); похоже, он обрезан."
                : "ответ не разобран как JSON; похоже, он обрезан.";
            return false;
        }
