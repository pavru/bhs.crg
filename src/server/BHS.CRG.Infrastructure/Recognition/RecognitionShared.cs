...
public static class RecognitionShared
{
    ...
    public static string BuildTitleBlockPrompt(IReadOnlyList<RecognitionField> fields)
    {
        ...
        if (fields.Any(f => f.Path == "ТипСтраницы"))
        {
            ...
            sb.AppendLine("- Обложка — декоративная/титульная страница ВСЕГО комплекта (обычно без штампа, может быть первой страницей).");
            sb.AppendLine("- ТитульныйЛист — страница с перечнем состава комплекта/документов (тоже обычно без постраничного штампа).");
            sb.AppendLine("- Документ — обычный лист с заполненным штампом (это подавляющее большинство страниц).");
            sb.AppendLine("Если сомневаешься — выбирай Документ.");
        }
        return sb.ToString();
    }
