...
public static class RecognitionShared
{
        ...
        out IReadOnlyDictionary<string, string?> values, out string? problem)
    {
        ...
        catch (JsonException ex)
        {
            ...
            // него — только место обрыва, и оно называется своими словами.
            problem = ex.BytePositionInLine is { } at
                ? $"ответ не разобран как JSON (оборвался на позиции {at}); похоже, он обрезан."
