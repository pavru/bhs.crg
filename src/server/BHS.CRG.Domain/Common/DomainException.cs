namespace BHS.CRG.Domain.Common;

/// <summary>
/// Отказ, сформулированный НАМИ: текст написан для пользователя и доходит до него дословно.
///
/// Отдельный тип заведён не ради стройности, а потому что отличить свой отказ от чужого иначе
/// нельзя. Раньше отказы бросались framework-типами (<c>ArgumentException</c>,
/// <c>KeyNotFoundException</c>, <c>InvalidOperationException</c>), а обработчик отдавал их текст
/// клиенту дословно — так и задумано для наших сообщений. Но ровно эти же типы бросают Npgsql, SDK
/// хранилища, Jint и разбор CIDR, и их сообщения называют хост, базу, бакет и имя параметра.
/// Различить их по типу невозможно: тип один и тот же.
///
/// Правило теперь обратное прежнему: наружу дословно уходит ТОЛЬКО потомок этого класса, всё
/// остальное — обобщённый ответ с идентификатором запроса, а подробности в лог. Значит, забыть
/// пометить свой отказ этим типом безопасно (пользователь увидит обобщённый текст), а вот
/// обратная ошибка — пометить им чужое сообщение — требует осознанного действия.
///
/// Код ответа выбирает слой API: здесь называется РОД отказа, а не HTTP-статус — Domain про HTTP
/// не знает и знать не должен.
///
/// <paramref name="inner" /> нужен там, где наш отказ ВЫВЕДЕН из чужой ошибки: пользователю уходит
/// наш текст, а в лог — исходное исключение целиком. Без него, например, отказ «задача уже
/// выполняется», рождённый нарушением уникального индекса, не оставлял бы в журнале ни имени
/// индекса, ни текста базы — то есть ничего, по чему разбирать столкновение.
/// </summary>
public abstract class DomainException(string message, Exception? inner = null) : Exception(message, inner);

/// <summary>
/// Текст отказа для человека — общий для всех выходов наружу, не только для HTTP-ответа.
///
/// Выходов больше одного: кроме ответа на запрос это запись фоновой задачи, уведомление в
/// колокольчике и поле ошибки в журнале сверки. Каждый из них показывает текст человеку, и правило
/// у всех обязано быть одно — иначе закрытая дверь соседствует с открытой.
/// </summary>
public static class Refusals
{
    /// <param name="fallback">Чем заменить чужое сообщение: обобщённый текст с меткой для журнала.</param>
    public static string TextOr(Exception? ex, string fallback) =>
        ex is DomainException domain ? domain.Message : fallback;
}

/// <summary>
/// Данные запроса недопустимы — исправить может тот, кто их прислал.
///
/// Роды отказа не запечатаны: свой тип с говорящим именем (скажем, «вёрстка не уложилась в срок»)
/// наследуется от подходящего рода и получает его код ответа — важен род, а не класс.
///
/// <paramref name="inner" /> — для отказов, ВЫВЕДЕННЫХ из чужой ошибки (issue #1050). Соблазн там
/// один и тот же: подклеить <c>ex.Message</c> к своему тексту, потому что иначе причина пропадает.
/// Тип при этом остаётся нашим, то есть чужое сообщение уходит наружу дословно — ровно то, что
/// правило запрещает, только через нашу же дверь. Исходное исключение кладут сюда: пользователь
/// видит наш текст, журнал получает причину целиком.
/// </summary>
public class InvalidRequestException(string message, Exception? inner = null)
    : DomainException(message, inner);

/// <summary>
/// Запрошенного объекта нет.
///
/// Сообщение по умолчанию есть, потому что «не найдено» чаще всего бросается без слов: ответ 404
/// говорит сам за себя, и половина мест в коде так и написана — <c>?? throw new NotFoundException()</c>.
/// Раньше в этом случае наружу уходил английский текст фреймворка про отсутствующий ключ словаря.
/// </summary>
public class NotFoundException(string message = "Запрошенный объект не найден.")
    : DomainException(message);

/// <summary>
/// Состояние не допускает операцию: объект занят, имя занято, порядок действий нарушен.
/// Отличается от <see cref="InvalidRequestException" /> тем, что запрос сам по себе правильный —
/// не пускает текущее положение дел.
/// </summary>
public class ConflictException(string message, Exception? inner = null) : DomainException(message, inner);

/// <summary>Действие запрещено этому пользователю.</summary>
public class ForbiddenException(string message) : DomainException(message);

/// <summary>
/// Место, на которое указывает отказ: путь в данных, код рода находки и текст для человека.
/// </summary>
public record RefusalDetail(string Code, string Path, string Message);

/// <summary>
/// Отказ, который называет МЕСТА, а не только причину (issue #957).
///
/// Заведено интерфейсом, а не полем конкретного класса, чтобы конвейер ответа не знал ни одного
/// отказа поимённо: он спрашивает «есть ли адреса», а не «ты ли та самая охрана записи».
/// Нужно это там, где нарушений может быть несколько и лежат они внутри структуры: отказ без
/// адреса превращается в баннер, по которому неверное значение внутри строки таблицы не найти —
/// её ещё надо догадаться открыть.
/// </summary>
public interface IDetailedRefusal
{
    IReadOnlyList<RefusalDetail> Details { get; }
}
