...
public class BugReportIssueTextTests
{
    ...
    /// <summary>
    /// Длинный стек в заготовке обрезается — и об этом сказано.
    ///
    /// Не ради красоты: у тела issue есть потолок (65 536 символов), а стек приезжает от клиента
    /// с запасом до 128 КБ. Заготовка, не влезающая в потолок, означала бы отказ отправки на уже
    /// разобранном сообщении — причём отказ чужими словами, ответом 422 «содержимое не годится».
    /// </summary>
    [Fact]
    public void Build_CutsAnOverlongStack_AndSaysSo()
    {
        var tech = Json($$"""{"stack":"{{new string('x', 20000)}}"}""");

        var text = BugReportIssueText.Build("Белый экран.", tech, hasScreenshot: false);

        Assert.True(text.Length < 20000, $"заготовка не обрезана: {text.Length} символов");
        Assert.Contains("стек обрезан", text);
    }
