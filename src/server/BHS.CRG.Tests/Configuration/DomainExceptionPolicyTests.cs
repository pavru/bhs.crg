...
public class DomainExceptionPolicyTests
{
    ...
    private static readonly Dictionary<string, string> DeliberatelyFramework = new()
    {
        ...
        ["BHS.CRG.Application/Schema/TagCatalog.cs"] = "два объявления одного кода тэга: реестр собирается при СТАРТЕ из кода ядра и модулей, до пользователя такой отказ не доходит вовсе — приложение просто не поднимается, назвав оба объявления (issue #959)",
    };
