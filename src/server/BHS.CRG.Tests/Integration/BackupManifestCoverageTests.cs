...
public class BackupManifestCoverageTests(IntegrationTestFixture fixture)
{
    ...
    private static readonly Dictionary<string, string> DeliberatelyExcluded = new()
    {
        ...
        // Права объявлены КОДОМ и сверяются с базой при каждом старте (AUTH-1). В копии они были бы
        // слепком чужой сборки: восстановили копию с экземпляра, где включены другие модули, — и
        // получили список прав, которого в этой сборке нет. Восстанавливать нечего: первый же старт
        // соберёт справочник заново, а роли ссылаются на коды, и коды переживают перенос.
        ["Permission"] = "справочник из кода: сверяется с объявлениями при каждом старте",
        ...
        // Предпочтения пользователя (issue #953, ТЗ CORE-25.3): тема, язык, а дальше пространства и
        // закреплённые разделы. В копию не идут по той же причине, что и сами учётные записи, —
        // строка настройки адресует пользователя, а пользователей копия не переносит. Приехавшая
        // настройка указывала бы на несуществующего человека; на целевой системе применить её не к
        // кому, а потерять нечего: тему человек выбирает заново одним переключателем.
        ["UserSetting"] = "предпочтения привязаны к учётной записи, а учётные записи копия не переносит",
