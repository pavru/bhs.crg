...
///
/// ⚠️ Подъём хоста НЕ безобиден: старт прогоняет ВСЕ свои разовые действия, а не только уборку
/// задач. Ощутимее прочих — «аккаунты без роли получают Admin» (<c>Program</c>, рядом с сидом
/// ролей): таблицы Identity <see cref="IntegrationTestFixture.ResetDatabaseAsync" /> намеренно не
/// чистит, учётные записи копятся между классами, и старт раздаёт им роль заново. До этого класса
/// старт случался ровно один раз — до всех тестов, — и <c>UpdateNotifierTests</c> прямо на это
/// рассчитывает («роли снимаются только в рамках теста»). Сейчас ничего не ломается: соседи создают
/// своих пользователей уже после старта либо снимают роли сами. Но если появится проверка, которой
/// важно, КТО держит Admin, — она может встретить не то, что оставила, и причина будет здесь.
/// </summary>
