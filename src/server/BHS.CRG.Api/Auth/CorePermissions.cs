...
public static class CorePermissions
{
    ...
    /// <summary>
    /// Сверка на непротиворечивость: прогон, разбор находок, алиасы позиций И правка определений
    /// (ТЗ CORE-37).
    ///
    /// Одно право на всю сверку — решение владельца 21.09.2026 (issue #992). Границу между
    /// «прогнать» и «править определение» рассматривали и отклонили: сверки ведут те же люди, что
    /// их настраивают, и второе право пришлось бы всегда выдавать парой с первым.
    ///
    /// ⚠️ Цена решения названа в объяснении права вслух: кто может завести определение, тот
    /// сравнивает любые наборы данных и читает их значения в находках. Ворота на модули это не
    /// отменяет — определение прогоняется загрузчиком без пользователя. Не выдавайте это право как
    /// «посмотреть расхождения»: оно шире.
    /// </summary>
    ...
    [
        ...
        new(ReconciliationRun,
            "заводить сверки, запускать их и разбирать находки",
            ...
            "значения любых наборов данных: правило сверки пишет сам пользователь, а находки "
                + "показывают сравниваемые величины",
