...
public static class EnumTypeEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/enum-types").RequireAuthorization(AppPolicies.Permission(CorePermissions.TypesRead));
        var admin = app.MapGroup("/api/enum-types").RequireAuthorization(AppPolicies.Permission(CorePermissions.TypesEdit));
