    ...
    public static void MapBackupEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            BackupFileStore store, IJobService jobs, ClaimsPrincipal user, CancellationToken ct) =>
        {
            ...
            if (await jobs.HasActiveOfKindAsync(JobKind.CreateBackup, ct))
                ...
                {
                    // Про пилюлю задач здесь не говорим: копию мог начать не человек, а расписание,
                    // и тогда никакой пилюли нет — список активных задач показывает только свои.
                    // Обещание «ход виден там-то» указывало бы на пустое место.
                    error = "Копия уже снимается — вручную или по расписанию. Дождитесь окончания: " +
