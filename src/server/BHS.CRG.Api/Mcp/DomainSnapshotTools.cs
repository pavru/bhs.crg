...
public class DomainSnapshotTools(IDomainSnapshotService domain, IHttpContextAccessor http)
{
    ...
    [Description("""
        ...
        sourceId (и datasetId), rowCount, origin. У привязанного поля это число строк после фильтра
        источника, у непривязанного — длина массива в самих реквизитах (такие таблицы заполняют
        руками либо наследуют). rowCount=null означает «сосчитать не удалось» или «значения нет».
        За строками привязанного поля идите в get_rows по sourceId.

        origin говорит, КАК получены строки источника: Parsed — детерминированный разбор файла,
        Recognized — прочитано моделью со скана (значения вероятностны, расхождение здесь может быть
        ошибкой чтения, а не расхождением в самих данных), System — консолидация данных системы.
        У непривязанного поля origin отсутствует.
