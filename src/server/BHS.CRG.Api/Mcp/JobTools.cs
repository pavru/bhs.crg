...
public class JobTools(IJobService jobs, IHttpContextAccessor http)
{
    ...
    // ⚠️ ЕДИНСТВЕННЫЙ инструмент без права (issue #948), и это решение, а не пропуск. Он отвечает
    // данными САМОГО пользователя: чужая задача для спрашивающего не существует — IJobService
    // сверяет владельца и отвечает отказом «задача не найдена». Тем же правилом живёт /api/jobs.
    //
    // Право «смотреть свои задачи» пришлось бы выдавать каждой роли и никогда не снимать — запись,
    // которая ничего не разграничивает, но которую сопровождают вечно. А без него запуск операции
    // был бы доступен, а её итог — нет.
