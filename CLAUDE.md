...
| БД | PostgreSQL 18 |
| Blob-хранилище | Garage на дев-стенде и в CI, MinIO в поставке — идёт переход, issue #885 |
...
работами. Node в CI — той же версии, что в `deploy/Dockerfile.web`. Обязательными в настройках
...
дамп рабочей базы невозможен: репозиторий публичный) и гоняют все восемь прогонов, 79 проверок из
82. Ручными остались три — разбиение PDF: им нужны распознанные страницы, то есть ИИ-движок,
...
**Образ хранилища — копия в нашем GHCR** (issue #882): `minio/minio` архивирован 13.02.2026,
community-версия больше не собирается. `ghcr.io/pavru/minio` и `ghcr.io/pavru/mc` — побайтовые
копии последнего выпуска с навсегда замороженным тегом; upstream остаётся только историей. Отбор
«наших» образов в `update.sh --gc` идёт поэтому по имени `bhs.crg-*`, а не по адресу реестра.
Замена хранилища (Garage / RustFS / своё файловое за `IBlobStorage`) — открытая часть #882.

**Хранилище переезжает на Garage** (issue #885, этап 1 из двух). Дев-стенд и живые прогоны CI
работают на `dxflrs/garage`; поставка пока на MinIO — её переводит этап 2, вместе с миграцией
данных ВНУТРИ `update.sh`. Пока переход не завершён, сторож образов в CI сверяет с поставкой только
базу, а версию Garage — между дев-compose и `deploy/Dockerfile.garage-init` (клиент и сервер
обязаны быть одной версии).

Что это значит на практике: хранилище слушает **3900**, ключи — вида `GK`+24 hex (`minioadmin`
больше не подходит), а перед работой Garage надо ИНИЦИАЛИЗИРОВАТЬ — раскладка, ключ, бакет. Делает
это `garage-init` из `deploy/garage-init.sh`, поднимается вместе со стендом и идемпотентен. Свой
образ у него потому, что в образе Garage нет shell.
