...
Под присмотром **весь рукописный код**: `src`, `deploy` (в том числе `update.sh` и `install.sh` —
самые большие файлы репозитория), `docs/tools`. Генерируемое — миграции EF, `node_modules`, `dist` —
не считается вовсе.
